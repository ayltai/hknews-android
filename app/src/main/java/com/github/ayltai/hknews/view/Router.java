...
public abstract class Router implements Disposable {
        ...
        return Flow.configure(newBase, this.activity)
            ...
            .dispatcher(KeyDispatcher.configure(this.activity, (outgoingState, incomingState, direction, incomingContexts, callback) -> {
                ...
                if (presenter != null && view != null) {
                    if (direction != Direction.BACKWARD && !DevUtils.isRunningInstrumentedTest() && view instanceof Cacheable) this.cache.put(incomingState.getKey(), Pair.create(new SoftReference<>(presenter), new SoftReference<>(view)));
    ...
    public boolean handleBack() {
        ...
        final View view = container.getChildAt(0);
        if (view instanceof Screen) return ((Screen)view).handleBack();
    ...
    private void dispatch(@Nonnull @NonNull @lombok.NonNull final View toView, @Nullable final State outgoingState, @Nonnull @NonNull @lombok.NonNull final State incomingState, @Nonnull @NonNull @lombok.NonNull final Direction direction, @Nonnull @NonNull @lombok.NonNull final TraversalCallback callback) {
        ...
        if (fromView == toView) {
            ...
            if (direction == Direction.BACKWARD) {
                ...
                this.dispatch(container, fromView, toView, direction);
            } else {
    ...
    private void dispatch(@Nonnull @NonNull @lombok.NonNull final ViewGroup container, @Nullable final View fromView, @Nonnull @NonNull @lombok.NonNull final View toView, @Nonnull @NonNull @lombok.NonNull final Direction direction) {
        if (fromView != null) {
            if (AnimationUtils.areAnimatorsEnabled()) {
                final Animator animator = this.getAnimator(direction == Direction.BACKWARD ? fromView : toView, direction, null, () -> container.removeView(fromView));
                if (animator == null) {
    ...
    private <V extends Presenter.View> void subscribe(@Nullable final Presenter<V> presenter, @Nullable final V view) {
        if (presenter != null && view != null) {
            this.manageDisposable(view.attaches(), view.attaches().subscribe(
                isFirstAttachment -> presenter.onViewAttached(view),
                RxUtils::handleError
            ));
